...
abstract class PulsarSourceSuiteBase extends PulsarSourceTest {
 ...
 class PulsarSourceTTLSuiteBase extends PulsarSourceTest {
  import PulsarOptions._
  import SchemaData._
  import testImplicits._
  ...
  test("test data loss") {
    ...
    Utils.tryWithResource(PulsarAdmin.builder().serviceHttpUrl(adminUrl).build()) { admin =>
      logInfo(s"Creating Pulsar source serviceUrl: ${serviceUrl} adminUrl: ${adminUrl}")

      //      logInfo("!--- setting namespace message TTL")
      //      admin.namespaces().setNamespaceMessageTTL("public/default", 5)
      ...
      try {
        withTempPaths(2) {
          case Seq(checkpointDir, checkpointDir2) =>
            ...
            //              """
            //                | {
            //                |              "persistent://public/default/topic-0" : [ 8, 3, 16, 9, 48, 0 ]
            //                |            },
            //                |""".stripMargin
